...
export class WalletProvider {
    ...
    constructor(args: Types.Class[], config: providerInterfaces.WalletProviderConfig) {
        this._classes = args
        this._config = config
        this._logger = config.logger || null
        this._walletProviderLogger = debug('WalletProvider:')
        this._walletProviderLogger.enabled = (config.logger && config.logger.debug) || false
    }
    ...
    connect(walletType: WALLET_TYPE): void {
        ...
        if ((Object.values(WALLET_TYPE) as string[]).includes(walletType) === false) {
        ...
        if (this._prevType !== this._currentType) {
            ...
            for (let index: number = 0; index < this._classes.length; index++) {
                ...
                if (Provider.prototype.type && Provider.prototype.type === walletType) {
                    if (Provider.prototype.type === WALLET_TYPE.WALLET_CONNECT) {
                        this._container[walletType] = new Provider(this._config.walletConnectConfig, this._logger)
                    } else {
            ...
            this._walletProviderLogger('» Connect new wallet:  %c' + this._currentType, 'color: #FABB51; font-size:14px')
        } else {
            // nothing
            this._walletProviderLogger('» Continue to connect the current wallet:  %c' + this._prevType, 'color: #FABB51; font-size:14px')
        }
    }
    ...
    destroy(type?: WALLET_TYPE): void {
        try {
            ...
            this._walletProviderLogger(
                '» The current instance is deleted before connecting new instance of the wallet: %c' + this._currentType,
