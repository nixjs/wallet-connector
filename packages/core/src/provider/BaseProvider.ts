...
export abstract class BaseProvider {
  ...
  private _etherProvider: Types.Nullable<ethers.ethers.providers.Web3Provider> =
    null;
  ...
  protected async _getTokenDecimalApi(
    contractAddress: string
  ): Promise<number> {
    const contractAbiFragment: ethers.utils.FunctionFragment =
      Utils.onGetFunctionAbi(ERC20, "decimals");
    const contract: ethers.Contract = new ethers.Contract(contractAddress, [
      contractAbiFragment,
    ]).connect(this.etherSigner);
  ...
  async getTokenBalance(
    ...
    try {
      const contractAbiFragment: ethers.utils.FunctionFragment =
        Utils.onGetFunctionAbi(ERC20, "balanceOf");
      const contract: ethers.Contract = new ethers.Contract(contractAddress, [
        contractAbiFragment,
      ]).connect(this.etherSigner);
      ...
      const bl: ethers.BigNumber =
        b && b.length > 0 ? b[0] : ethers.BigNumber.from(0);
  ...
  async getTransaction(
    ...
    Interfaces.ResponseData<
      ethers.providers.TransactionResponse | TronTypes.TransactionResponse
    >
  > {
    try {
      const txIn: ethers.providers.TransactionResponse =
        await this.etherProvider.getTransaction(transactionHash);
  ...
  async getTransactionReceipt(
    ...
    try {
      const txIn: ethers.providers.TransactionReceipt =
        await this.etherProvider.getTransactionReceipt(transactionHash);
  ...
  > {
    try {
      const txIn: ethers.providers.TransactionReceipt =
        await this.etherProvider.getTransactionReceipt(transactionHash);
      const logs: ethers.utils.LogDescription[] = Utils.onDecodeTransactionLog(
        ABIs,
        txIn
      );
  ...
  async sendTransaction(
    transactionRequest:
      ...
      | TriggerSmartContractTypes.TriggerSmartContractRequest
  ): Promise<
    Interfaces.ResponseData<
      ethers.providers.TransactionResponse | TronTypes.TransactionResponse
    >
  > {
    try {
      const txIn: ethers.providers.TransactionResponse =
        await this.etherSigner.sendTransaction(
  ...
  > {
    try {
      const tx: Types.Deferrable<ethers.providers.TransactionRequest> =
        options || {};
      tx.to = to;
      ...
      const txIn: ethers.providers.TransactionResponse =
        await this.etherSigner.sendTransaction(tx);
  ...
  async sendToken(
    ...
    to: string,
    options?:
      | providerInterfaces.TransactionRequest
      | TriggerSmartContractTypes.Option
  ): Promise<
    Interfaces.ResponseData<
      ethers.providers.TransactionResponse | TronTypes.TransactionResponse
    >
  > {
    try {
      const decimalAbi: ethers.utils.FunctionFragment = Utils.onGetFunctionAbi(
        ERC20,
        "decimals"
      );
      const transferAbi: ethers.utils.FunctionFragment = Utils.onGetFunctionAbi(
        ERC20,
        "transfer"
      );
      const contractAbiFragment: ethers.utils.FunctionFragment[] = [
      ...
      if (ourOptions.maxPriorityFeePerGas) {
        tx.maxPriorityFeePerGas = ethers.utils.hexlify(
          ourOptions.maxPriorityFeePerGas
        );
      }
