...
export class TronLinkProvider extends BaseProvider {
  ...
  > {
    try {
      ...
      const result: TronTypes.ResponseData<TronTypes.Account> =
        await TronLinkRequest.getAccountApi(node.host, from);

      const {
        data,
        status,
        error,
      }: TronTypes.ResponseParser<TronTypes.Account | TronTypes.ResponseError> =
    ...
    try {
      ...
      if (data?.constantResult && data.constantResult.length > 0) {
        return {
          status: "SUCCESS",
          data: Number(
            Helpers.fromDecimal(`0x${data.constantResult[0]}`).toString()
          ),
        };
      }
  ...
  async getTokenBalance(
    ...
    try {
      ...
      if (data?.constantResult && data.constantResult.length > 0) {
        return {
          status: "SUCCESS",
          data: Helpers.fromHexParser(
            ethers.BigNumber.from(`0x${data.constantResult[0]}`).toNumber(),
            dec
          ),
        };
      }
  ...
  async getTransaction(
    ...
    try {
      ...
      const result: TronTypes.ResponseData<TronTypes.TransactionResponse> =
        await TronLinkRequest.getTransactionApi(node.host, transactionHash);

      const {
        data,
        status,
        error,
      }: TronTypes.ResponseParser<
        TronTypes.TransactionResponse | TronTypes.ResponseError
  ...
  async sendTransaction(
    ...
    try {
      ...
      const broadcastTx = await this.walletInstance.trx.sendRawTransaction(
        signature
      );
  ...
  async sendNativeToken(
    ...
    try {
      if (options) {
        console.warn(
          "The `options` parameter has been deprecated for TronLink Provider"
        );
      }
      const from: string = this.walletInstance.defaultAddress.base58;
      const unSignedTxn = await this.walletInstance.transactionBuilder.sendTrx(
        to,
        amount,
        from
      );
  ...
  async triggerSmartContractApi(
    ...
    Interfaces.ResponseData<{
      transaction: TronTypes.TransactionResponse;
  ...
  > {
    try {
      ...
      } = await this.walletInstance.transactionBuilder.triggerSmartContract(
        contractAddress,
        functionCall,
        options,
        parameter,
