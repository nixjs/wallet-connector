...
export class TronLinkProvider extends BaseProvider {
    ...
    protected async config(): Promise<Interfaces.ResponseData<string>> {
        try {
            ...
            this.log(`» 🚀 Established connection successfully to %${this.context}`, 'color: #FABB51; font-size:14px')
            return {
                data: +this.walletInstance.defaultAddress.base58 === 0 ? '' : this.walletInstance.defaultAddress.base58,
                status: 'SUCCESS'
            }
        } catch (error) {
            return {
    ...
    async getTokenDecimal(contractAddress: string): Promise<Interfaces.ResponseData<number>> {
        try {
            ...
            if (data?.constantResult && data.constantResult.length > 0) {
                return {
                    status: 'SUCCESS',
                    data: Number(Helpers.fromDecimal(`0x${data.constantResult[0]}`).toString())
                }
            }
