...
function verifyChecksum(hexAddress: string): boolean {
    try {
        ...
        const addressHash1: string = ethers.utils.keccak256(expanded).replace('0x', '')

        for (let i: number = 0; i < 40; i++) {
            // The nth letter should be uppercase if the nth digit of casemap is 1
            if (
                (parseInt(addressHash1[i], 16) > 7 && addressHash0[i].toUpperCase() !== addressHash0[i]) ||
                (parseInt(addressHash1[i], 16) <= 7 && addressHash0[i].toLowerCase() !== addressHash0[i])
            ) {
                return false
            }
        }
...
function isAddressValid(hexAddress: string): boolean {
    ...
    if (/^0x[0-9a-f]{40}$/.test(hexAddress) || /^0x?[0-9A-F]{40}$/.test(hexAddress)) {
