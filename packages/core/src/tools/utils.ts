...
import { WALLET_TYPE, CHAIN_TYPE_MAINNET, CHAIN_TYPE_TESTNET, NETWORK_WALLET, FEE_CONFIG, TOKEN_TYPE } from '../constants'
...
export class Utils {
    static onGetWalletInstalledBy(walletType: Types.Nullable<WALLET_TYPE>): WalletInstalledType {
    ...
    static onGetWalletInstalled(walletType: Types.Nullable<WALLET_TYPE>): (WalletInstalledType | '')[] {
        if (walletType) {
            return [this.onGetWalletInstalledBy(walletType)]
        }

        return Object.keys(WALLET_TYPE).map((w: string) => w && this.onGetWalletInstalledBy(WALLET_TYPE[w as WALLET_TYPE]))
    }
    ...
    static onGetNetwork(chainId: any): Types.Nullable<NETWORK_WALLET> {
        ...
        const cId: ChainIdType<[number, string]> = (Helpers.isHex(chainId) && Helpers.toNumberFromHex(chainId)) || chainId
    ...
    static onGetFunctionAbi(abi: JsonFragment[], functionName: string): ethers.utils.FunctionFragment {
        const iface: ethers.utils.Interface = new ethers.utils.Interface(abi)
        return iface.getFunction(functionName)
    }

    static onGetEventAbi(abi: JsonFragment[], eventName: string): ethers.utils.EventFragment {
        const iface: ethers.utils.Interface = new ethers.utils.Interface(abi)
        ...
        receipt.logs.forEach((log: { topics: Array<string>; data: string }) => {
            Object.keys(ABIs).forEach((key: string) => {
                try {
                    ...
                    const iface: ethers.utils.Interface = new ethers.utils.Interface(fragment)
    ...
    static onEstimateFee(network: NETWORK_WALLET, type: TOKEN_TYPE): Types.Nullable<number> {
        const state: Record<TOKEN_TYPE, Record<string, number>> = FEE_CONFIG[network as Exclude<NETWORK_WALLET, NETWORK_WALLET.BNB>]
